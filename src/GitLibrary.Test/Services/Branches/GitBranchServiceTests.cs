...
namespace GitLibrary.Test.Services.Branches;

public class GitBranchServiceTests(ITestOutputHelper output)
{
    [Fact]
    public async Task GetCurrentBranchNameAsync_ReturnsCheckedOutBranch()
    {
        ...
        // Act
        var name = await git.Branches.GetCurrentBranchNameAsync();
    ...
    public async Task GetCurrentBranchNameAsync_ThrowsOperationCanceled_WhenCancelled()
    {
        ...
        //Act
        var ex = await Record.ExceptionAsync(() => git.Branches.GetCurrentBranchNameAsync(cancellationTokenSource.Token));
